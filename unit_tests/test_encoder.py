...
from encoder.move_encoder import CORE_FEATURES_DIM, ENCODED_MOVE_DIM, MOVE_CATEGORIES, SECONDARY_EFFECTS_DIM, SPECIAL_CASES_DIM, VOLATILE_STATUSES, MoveEncoder
...
def test_volatile_status_prob_dim():
    assert MoveEncoder._volatile_status_prob(Move("tackle", gen=9)).shape == (len(VOLATILE_STATUSES),)

def test_encode_category_dim():
    assert MoveEncoder._encode_category(Move("tackle", gen=9)).shape == (len(MOVE_CATEGORIES),)

def test_encode_core_features():
    assert MoveEncoder._encode_core_features(Move("tackle", gen=9)).shape == (CORE_FEATURES_DIM,)

def test_encode_self_stat_change_dim():
    assert MoveEncoder._encode_self_stat_change(Move("tackle", gen=9)).shape == (len(MOVABLE_STATS),)

def test_encode_foe_stat_change_dim():
    assert MoveEncoder._encode_foe_stat_change(Move("tackle", gen=9)).shape == (len(MOVABLE_STATS),)

def test_encode_secondary_effects_dim():
    assert MoveEncoder._encode_secondary_effects(Move("tackle", gen=9)).shape == (SECONDARY_EFFECTS_DIM,)

def test_encode_special_cases_dim():
    assert MoveEncoder._encode_special_cases(Move("tackle", gen=9)).shape == (SPECIAL_CASES_DIM,)
...
def test_encode_pokemon():
    ...
    assert PokemonEncoder.opponent_pokemon_encoder(mon).shape == (PokemonEncoder.OP_POKEMON_FEATURES_DIM,)
    assert PokemonEncoder.my_pokemon_encoder(mon).shape == (PokemonEncoder.MY_POKEMON_FEATURES_DIM,)
...
def test_encoder():
    ...
    print(f"Total features dim: {Encoder.MY_TEAM_FEATURES_DIM + Encoder.OP_TEAM_FEATURES_DIM}")