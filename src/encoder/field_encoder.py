...
class FieldEncoder:
    ENCODED_FIELD_DIM = len(TERRAINS) + len(HAZARDS) * 2 + len(WEATHERS)
    ...
    def encode_field(battle: Battle) -> NDArray[np.float32]:
        ...
        return np.concatenate(
            [
                ...
                FieldEncoder._encode_weather(battle),
            ]
        )

    @staticmethod
    def _encode_weather(battle: Battle) -> NDArray[np.float32]:
        return np.array(
            [1.0 if weather in battle.weather else 0.0 for weather in WEATHERS],
            dtype=np.float32,
        )
    ...
    def _encode_terrain(battle: Battle) -> NDArray[np.float32]:
        return np.array(
            [
                1.0 if battle.fields.get(terrain, None) is not None else 0.0
    ...
    def _encode_hazards(
        ...
        return np.array(
            [1.0 if side_conditions.get(hazard, 0) else 0.0 for hazard in HAZARDS],
            dtype=np.float32,
        )
